...
void showword(long addr) {
  ...
  case 'b':
    ...
    break;
  case 'd':
    printf("%5ld  %08lX  %s  %4ld", addr, word.data, wordtochars(charbuf, word),
           word.data);
    break;
  case 'u':
    printf("%5ld  ??", addr);
...
void showusage() {
  ...
  printf("The command line may contain source file names and options in any "
         "order.\n");
  printf("There should be at least one source file.  Source files will be "
         "loaded\n");
  ...
  printf("       Use +o or -o followed by a name to changes the list file "
         "name.\n");
}
